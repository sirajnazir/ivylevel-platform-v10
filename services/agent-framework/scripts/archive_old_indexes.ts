...
async function run(){
  ...
  if (cmd === 'list'){
    ...
    console.table(ix.indexes?.map((i:any)=>({ name: i.name, dimension: i.dimension, metric: i.metric })) || []);
    return;
  }
  if (cmd === 'delete'){
    const names = process.argv.slice(3);
    if (!names.length) throw new Error('Provide names to delete');
    for (const n of names){
      console.log(`[delete] ${n}`);
      try { await pc.deleteIndex(n); console.log('[ok]'); } catch (e){ console.error(`[fail] ${n}`, e);}
    }
    return;
  }
