-- Jenny Agentic AI - Universal Vitals + Progression + Query Model
-- Migration v3.0 - Complete DDL
-- Run in a single transaction in production

-- 0. Extensions (if you prefer uuid_generate_v4, adjust below)
...
EXCEPTION WHEN duplicate_object THEN NULL; END $;
DO $ BEGIN
  CREATE TYPE outcome_type AS ENUM ('admission','plan','tracking','momentum','artifact','draft','submission','result','milestone','ops','policy','registry','content_bank','communication','planning');
EXCEPTION WHEN duplicate_object THEN NULL; END $;
DO $ BEGIN
...
-- Fact kinds, tactic_name, framework as reference tables to allow growth without DDL
...
-- Lifecycle items for Tier1/Tier2 tracking
CREATE TABLE IF NOT EXISTS lifecycle_items (
  ...
  school TEXT, -- for application domain
...
-- Atomic facts (no overwrites; append only per date)
...
CREATE TABLE IF NOT EXISTS interactions (
  ...
  channel TEXT NOT NULL, -- e.g., chat, email, raw_transcript
  user_ask TEXT,
  jenny_reply TEXT,
...
CREATE TABLE IF NOT EXISTS outcomes (
  ...
  admission_result admission_result, -- nullable unless type='admission' or 'result'
...
-- Evidence links (chips) that can attach to facts/interactions/outcomes
CREATE TABLE IF NOT EXISTS evidence_links (
  evidence_id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  source_id TEXT NOT NULL REFERENCES sources(source_id) ON DELETE RESTRICT,
  snippet_id TEXT, -- optional: link to interactions.snippet_id
  offset_start INT,
  offset_end INT,
  quote TEXT,
  created_ts timestamptz DEFAULT now()
...
-- 3. Seed reference rows (idempotent upserts)
-- Fact kinds
