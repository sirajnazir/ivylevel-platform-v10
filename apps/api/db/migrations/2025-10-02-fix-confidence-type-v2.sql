-- Fix the confidence column type mismatch by dropping and recreating the functions

-- Drop both functions
DROP FUNCTION IF EXISTS select_current_facts(text, text[]);
DROP FUNCTION IF EXISTS select_current_fact(text, text);

-- Recreate select_current_fact with confidence as text
