...
for i in {1..30}; do
  RESPONSE=$(curl -s -X POST "${API_BASE}/api/kb-chat" \
    ...
    -d "{\"userMessage\":\"I got rejected from Stanford\",\"studentId\":\"$STUDENT_ID\",\"sessionId\":\"tone-$i\",\"topK\":3}")
...
for i in {1..30}; do
  RESPONSE=$(curl -s -X POST "${API_BASE}/api/kb-chat" \
    -H "Content-Type: application/json" \
    -d "{\"userMessage\":\"What awards did I win?\",\"studentId\":\"$STUDENT_ID\",\"sessionId\":\"fact-$i\",\"topK\":3}")
