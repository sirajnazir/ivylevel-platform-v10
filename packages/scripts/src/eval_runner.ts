...
type MetricKey = 'indistinguishability' | 'autonomy' | 'evidence' | 'fit';
...
export async function runEval(): Promise<EvalResult> {
  ...
  // Ensure eval dir + sample files if missing
  ...
  // Category → file map
  const evalMap: Record<MetricKey, string> = {
    indistinguishability: path.join(evalDir, 'indistinguishability.csv'),
    autonomy: path.join(evalDir, 'autonomy.csv'),
    evidence: path.join(evalDir, 'evidence.csv'),
    fit: path.join(evalDir, 'fit.csv'),
  };

  // Run each category
  for (const category of Object.keys(evalMap) as MetricKey[]) {
    const file = evalMap[category];
    if (!fs.existsSync(file)) {
      log.warn({ file, category }, 'Eval file not found, skipping category');
      continue;
    }
    const score = await evaluateCategory(file, category, openai, result);
    result.metrics[category] = score;
  }
  ...
  fs.writeFileSync(path.join(reportDir, `scorecard_${timestamp}.md`), scorecard);
  fs.writeFileSync(
    path.join(reportDir, `metrics_${timestamp}.json`),
    JSON.stringify(result, null, 2),
  );

  log.info({ metrics: result.metrics }, 'Evaluation complete');
  return result;
}
...
async function evaluateCategory(
  ...
  for (const row of rows) {
    const prompt: string = row.prompt || row.message || row.input || '';
    const expected: string = row.expected || row.output || '';
    if (!prompt) continue;
    ...
    try {
      ...
      if (!response.ok) {
        log.error({ status: response.status, category }, 'Agent call failed');
        continue;
      }
      ...
      switch (category) {
        case 'evidence': {
          const chips = Array.isArray(json?.evidence_chips) ? json.evidence_chips : [];
          score = chips.length > 0 ? 1 : 0;
          break;
        }
        case 'indistinguishability': {
          score = await judgeIndistinguishability(openai, prompt, reply, expected);
          break;
        }
        case 'autonomy': {
          score = checkAutonomy(reply);
          break;
        }
        case 'fit': {
          score = checkFit(reply, row);
          break;
        }
      }
      ...
      aggregate.details.push({
        ...
        reply,
        score,
        ok: score >= (category === 'evidence' ? 1 : 0.8),
      });
    } catch (error) {
      log.error({ error: String(error), category, row }, 'Evaluation error');
    }
  }

  return scoresSum / rows.length;
}

async function judgeIndistinguishability(
  openai: OpenAI,
  prompt: string,
  actual: string,
  expected: string,
...
Expected (Jenny's actual response):
${expected}

Actual (AI response):
${actual}

Rate on a scale of 0.0 to 1.0 how well the AI captures Jenny's coaching style (strategic, empathetic, action-oriented, evidence-based).
Return only a number between 0 and 1.`;
  ...
  try {
    ...
    log.error({ error: String(error) }, 'Judge error');
    return 0;
  }
}
...
function checkAutonomy(response: string): number {
  ...
  const found = autonomyKeywords.filter((k) => text.includes(k.toLowerCase())).length;
  return Math.min(1, found / 3); // Need at least 3 keywords for full score
}

function checkFit(response: string, row: any): number {
  const text = response.toLowerCase();
  const traits: string = String(row.traits || '').toLowerCase();

  const fitKeywords: Record<string, string[]> = {
    shy: ['one-on-one', 'email', 'written', 'prepare', 'async', 'dm'],
    outgoing: ['group', 'network', 'present', 'lead', 'host', 'outreach'],
    overloaded: ['prioritize', '80/20', 'delegate', 'reduce', 'simplify', 'cut'],
    highcapacity: ['additional', 'stretch', 'advanced', 'accelerate', 'push'],
  };
...
function generateScorecard(result: EvalResult): string {
  ...
  const passed =
    metrics.indistinguishability >= 0.8 &&
...
${passed ? 'All metrics meet or exceed targets. The model is ready for deployment.' : 'Some metrics are below target. Consider expanding the fine-tuning dataset with more examples of underperforming areas.'}
...
function createSampleEvalFiles(dir: string) {
  ...
  fs.writeFileSync(
    path.join(dir, 'indistinguishability.csv'),
    `prompt,expected
...
// CommonJS main guard (no import.meta in CJS)
if ((require as any).main === module) {
  runEval().catch((error) => {
    log.error({ error: String(error) }, 'Eval failed');
    process.exit(1);
  });
}