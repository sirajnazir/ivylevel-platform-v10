...
import { Users, GraduationCap, ArrowRight, Shield } from 'lucide-react';
import { StudentLogin } from '../auth/StudentLogin';
import { CoachLogin } from '../auth/CoachLogin';
import { AdminLogin } from '../auth/AdminLogin';

const EntryPortal: React.FC = () => {
  const [selectedRole, setSelectedRole] = useState<'student' | 'coach' | 'admin' | null>(null);
  ...
  useEffect(() => {
    ...
    if (roleParam === 'student' || roleParam === 'coach' || roleParam === 'admin') {
      setSelectedRole(roleParam);
      setShowLogin(true);
    }
  }, [searchParams]);

  const handleRoleSelection = (role: 'student' | 'coach' | 'admin') => {
    setSelectedRole(role);
    setShowLogin(true);
  };

  const handleLoginSuccess = () => {
    // Let the login components handle their own redirection
    // by not passing onSuccess callback
  };

  const handleBackToRoles = () => {
    setShowLogin(false);
    setSelectedRole(null);
    // Remove the role parameter from URL when going back
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-student-primary/5 to-coach-primary/5 flex items-center justify-center p-4">
      <div className="max-w-4xl w-full">
        <div className="text-center mb-12">
          <div className="flex justify-center mb-6">
            <svg style={{width: '92px', height: '27px'}} viewBox="0 0 92 27" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <h1 className="text-4xl font-bold text-gray-900 mb-4">Welcome to Ivylevel</h1>
          <p className="text-xl text-gray-600">Your Personalized College Preparation Platform</p>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-8">
          ...
          <div 
            ...
            className="bg-white rounded-2xl shadow-lg p-8 cursor-pointer transform transition-all hover:scale-105 hover:shadow-xl border-2 border-transparent hover:border-student-primary"
          >
            <div className="flex flex-col items-center text-center">
              <div className="w-20 h-20 bg-student-primary/10 rounded-full flex items-center justify-center mb-4">
                <GraduationCap className="w-10 h-10 text-student-primary" />
              </div>
              <h2 className="text-2xl font-bold text-gray-900 mb-2">I'm a Student</h2>
              <p className="text-gray-600 mb-6">Access your personalized college prep dashboard</p>
              <div className="flex items-center text-student-primary font-medium">
          ...
          <div 
            ...
            className="bg-white rounded-2xl shadow-lg p-8 cursor-pointer transform transition-all hover:scale-105 hover:shadow-xl border-2 border-transparent hover:border-coach-primary"
          >
            <div className="flex flex-col items-center text-center">
              <div className="w-20 h-20 bg-coach-primary/10 rounded-full flex items-center justify-center mb-4">
                <Users className="w-10 h-10 text-coach-primary" />
              </div>
              <h2 className="text-2xl font-bold text-gray-900 mb-2">I'm a Coach</h2>
              <p className="text-gray-600 mb-6">Access your coaching dashboard and resources</p>
              <div className="flex items-center text-coach-primary font-medium">
                Enter as Coach <ArrowRight className="w-4 h-4 ml-2" />
              </div>
            </div>
          </div>
          ...
          <div 
            ...
            className="bg-white rounded-2xl shadow-lg p-8 cursor-pointer transform transition-all hover:scale-105 hover:shadow-xl border-2 border-transparent hover:border-red-600"
          >
            <div className="flex flex-col items-center text-center">
              ...
              <p className="text-gray-600 mb-6">Manage platform settings and users</p>
        ...
        <div className="mt-12 text-center">
          <p className="text-sm text-gray-500">
            Need help? Contact support at support@ivylevel.com
          </p>
        </div>
      </div>
    </div>
