...
// Brand Logo Components (consistent with student/coach login)
...
export const AdminLogin: React.FC<AdminLoginProps> = ({ onSuccess, onBack }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (result.success) {
      ...
      if (result.user) {
        console.log('Updating auth context with admin user:', result.user);
        localStorage.setItem('user', JSON.stringify(result.user));
        localStorage.setItem('token', result.tokens?.access_token || '');
        
        // Update the auth context
        const contextResult = await authLogin(email, password);
        console.log('Auth context update result:', contextResult);
      }
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-red-900 flex items-center justify-center p-4">
      <div className="max-w-md w-full">
        ...
        <div className="bg-white rounded-2xl shadow-2xl p-8">
          <div className="text-center mb-8">
            {onBack && (
              <button
                onClick={onBack}
                className="absolute top-4 left-4 text-gray-600 hover:text-gray-800 flex items-center space-x-2"
              >
          ...
          {/* Security Badge */}
          <div className="bg-gray-900 text-white rounded-lg p-4 mb-6 flex items-center space-x-3">
            <Shield className="w-6 h-6 text-red-400" />
            <div className="flex-1">
              <p className="text-sm font-semibold">Restricted Access</p>
              <p className="text-xs text-gray-300">Administrative privileges required</p>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="pt-2">
              <button
                type="submit"
                disabled={loading}
                className="w-full py-3 bg-gradient-to-r from-gray-800 to-red-900 text-white rounded-lg font-semibold hover:from-gray-900 hover:to-red-800 transition-all disabled:opacity-50"
              >
        ...
        <div className="mt-4 bg-white/10 backdrop-blur rounded-lg p-4 text-white">
          <p className="text-sm font-medium mb-2">Demo Admin Access:</p>
          <p className="text-xs">Email: admin@ivylevel.com</p>
