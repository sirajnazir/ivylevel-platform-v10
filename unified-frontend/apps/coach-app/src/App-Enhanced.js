...
import { 
  ...
  TrophyIcon, TargetIcon, VideoIcon, PlayIcon, CheckIcon,
  ArrowRightIcon, ICON_COLORS 
...
function App() {
  ...
  // Training state - moved to top level to fix React Hooks rules
    ...
    return (
      <div style={{minHeight: '100vh', background: '#f9fafb'}}>
        ...
        <div style={{maxWidth: '1200px', margin: '0 auto', padding: '32px 24px'}}>
          ...
          <p style={{textAlign: 'center', fontSize: '0.75rem', color: '#999', marginBottom: '16px'}}>
            v2.1 - Smart Onboarding & Training Connected - UPDATED {new Date().toLocaleTimeString()}
          </p>
          ...
          <div style={{display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '24px'}}>
            ...
            <div
              onClick={(e) => {
                e.stopPropagation();
                e.preventDefault();
                console.log('Card 1 clicked - setting view to onboarding');
                console.log('Current view before click:', view);
                console.log('DEBUG: This is the NEW code - v2.1');
                window.DEBUG_CLICK = 'Card 1 clicked at ' + new Date().toISOString();
                try {
                  setView('onboarding');
                } catch (error) {
            ...
            <div
              onClick={(e) => {
                e.stopPropagation();
                e.preventDefault();
                console.log('Card 2 clicked - setting view to training');
                console.log('Current view before click:', view);
                try {
                  setView('training');
                } catch (error) {
  ...
  // Training View - Simple interactive modules
  if (view === 'training') {
    
    const trainingModules = [
      { 
        id: 'welcome', 
        name: 'Welcome & IvyLevel Method', 
        icon: <TargetIcon size={20} color={ICON_COLORS.default} />,
        duration: '15 min',
        description: 'Understand the IvyLevel philosophy and your role as a coach'
      },
      { 
        id: 'gameplan', 
        name: 'Game Plan Mastery', 
        icon: <BookIcon size={20} color={ICON_COLORS.default} />,
        duration: '30 min',
        description: 'Master reading and implementing student Game Plans'
      },
      { 
        id: 'session', 
        name: 'Session Excellence', 
        icon: <VideoIcon size={20} color={ICON_COLORS.default} />,
        duration: '45 min',
        description: 'Learn to conduct impactful coaching sessions'
      },
      { 
        id: 'communication', 
        name: 'Communication Skills', 
        icon: <UserIcon size={20} color={ICON_COLORS.default} />,
        duration: '20 min',
        description: 'Build rapport with students and parents'
      },
      { 
        id: 'certification', 
        name: 'Final Assessment', 
        icon: <TrophyIcon size={20} color={ICON_COLORS.default} />,
        duration: '30 min',
        description: 'Complete your certification to start coaching'
      }
    ];

    const isModuleCompleted = (moduleId) => moduleScores[moduleId] >= 80;
    const allModulesCompleted = trainingModules.every(m => isModuleCompleted(m.id));
    ...
    return (
      <div style={{minHeight: '100vh', background: '#f9fafb'}}>
          ...
          <div style={{maxWidth: '1200px', margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between'}}>
            ...
            <div style={{display: 'flex', alignItems: 'center', gap: '16px'}}>
              <div style={{fontSize: '0.875rem', color: '#6b7280'}}>
                Progress: {Object.keys(moduleScores).length}/{trainingModules.length} modules
              </div>
              <button
                onClick={() => setView('home')}
                style={{
        ...
        <div style={{maxWidth: '1200px', margin: '0 auto', padding: '32px 24px'}}>
          <h1 style={{fontSize: '2rem', fontWeight: 'bold', marginBottom: '32px', color: '#641432'}}>
            Coach Training & Certification
          </h1>

          <div style={{display: 'grid', gridTemplateColumns: '300px 1fr', gap: '32px'}}>
            {/* Module List */}
            <div style={{background: 'white', borderRadius: '12px', padding: '24px', height: 'fit-content'}}>
              <h3 style={{marginBottom: '20px', fontWeight: '600'}}>Training Modules</h3>
              {trainingModules.map((module, idx) => (
                <div
                  ...
                  style={{
                    ...
                    padding: '12px',
                    marginBottom: '8px',
                    background: currentModule === idx ? '#FFE5DF' : '#f9fafb',
                    borderRadius: '8px',
                    cursor: 'pointer',
                    transition: 'all 0.2s'
                  }}
                >
                  <div style={{
                    width: '40px',
                    height: '40px',
                    background: isModuleCompleted(module.id) ? '#22c55e' : currentModule === idx ? ICON_COLORS.primary : '#e5e7eb',
                    borderRadius: '50%',
                    display: 'flex',
                    alignItems: 'center',
                    justifyContent: 'center'
                    ...
                    {isModuleCompleted(module.id) ? 
                      ...
                      React.cloneElement(module.icon, { color: currentModule === idx ? 'white' : '#6b7280' })
                    }
                  </div>
                  <div style={{flex: 1}}>
                    <div style={{fontWeight: '600', fontSize: '0.875rem'}}>{module.name}</div>
                    <div style={{fontSize: '0.75rem', color: '#6b7280'}}>{module.duration}</div>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div style={{background: 'white', borderRadius: '12px', padding: '32px'}}>
              <div style={{marginBottom: '24px'}}>
                <h2 style={{fontSize: '1.5rem', fontWeight: '600', marginBottom: '8px'}}>
                  {trainingModules[currentModule].name}
                </h2>
                <p style={{color: '#6b7280'}}>
              ...
              <div style={{
                ...
                <PlayIcon size={64} color="#e5e7eb" />
                <p style={{marginTop: '16px', color: '#6b7280', textAlign: 'center'}}>
                  Module content would include videos, interactive exercises, and quizzes
                </p>
              </div>
              ...
              <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}>
                <button
                  ...
                  style={{
                    padding: '12px 24px',
                    background: currentModule === 0 ? '#e5e7eb' : 'white',
                    color: currentModule === 0 ? '#9ca3af' : '#641432',
                    border: `1px solid ${currentModule === 0 ? '#e5e7eb' : '#641432'}`,
                    borderRadius: '8px',
                ...
                {!isModuleCompleted(trainingModules[currentModule].id) ? (
                  <button
                    onClick={() => {
                      // Simulate completing the module
                      setModuleScores({
                        ...moduleScores,
                        [trainingModules[currentModule].id]: 95
                      });
                  ...
                  <button
                    onClick={() => currentModule < trainingModules.length - 1 && setCurrentModule(currentModule + 1)}
                    disabled={currentModule === trainingModules.length - 1}
                    style={{
                      padding: '12px 32px',
                      background: currentModule === trainingModules.length - 1 ? '#22c55e' : ICON_COLORS.primary,
                      color: 'white',
                      border: 'none',
                      borderRadius: '8px',
                      cursor: 'pointer',
                  ...
                  >
                    {currentModule === trainingModules.length - 1 ? 
                      (allModulesCompleted ? 'Training Complete!' : 'Complete All Modules First') : 
                      <>Next Module <ArrowRightIcon size={20} color="white" /></>
                    }
                  </button>
                )}
              </div>
          ...
          {allModulesCompleted && (
            <div style={{
              ...
              <h3 style={{fontSize: '1.5rem', marginTop: '16px', marginBottom: '8px'}}>
                Congratulations! You're Certified!
              </h3>
              <p>You've completed all training modules and are ready to start coaching.</p>
            </div>
          )}
        </div>
      </div>
